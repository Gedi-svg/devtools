...
import { OAppAltUpgradeable, Origin } from "@layerzerolabs/oapp-alt-evm/contracts/oapp-upgradeable/OAppAltUpgradeable.sol";
...
/**
 * @title OFTAltCoreUpgradeable
 * @dev Abstract upgradeable contract for the OftChain (OFT) token using alternative payment methods.
 * @dev This maintains the same storage layout as OFTCoreUpgradeable but uses OAppAlt for non-native token payments.
 */
abstract contract OFTAltCoreUpgradeable is
    IOFT,
    OAppAltUpgradeable,
    ...
    OAppOptionsType3Upgradeable
{
     ...
     */
    constructor(uint8 _localDecimals, address _endpoint, address owner) OAppAltUpgradeable(_endpoint, owner) {
     ...
     */
    function __OFTAltCore_init(address _delegate) internal onlyInitializing {
        __OAppAlt_init(_delegate);
        __OAppPreCrimeSimulator_init();
        __OAppOptionsType3_init();
    }

    function __OFTAltCore_init_unchained() internal onlyInitializing {}
    ...
    function send(
        ...
        // @dev In Alt implementation, msg.value should always be 0 since we use ERC20 tokens for fees
